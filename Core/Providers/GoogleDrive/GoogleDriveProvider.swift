...
public actor GoogleDriveProvider: CloudProvider {
    ...
    public func validateCredentials(account: CloudAccount) async throws -> Bool {
        ...
        let request = HTTPRequest(
            url: URL(string: "\(Self.baseURL)/about?fields=storageQuota") ?? URL(fileURLWithPath: "/"),
    ...
    public func listDirectory(
        ...
        var queryItems = [
            ...
            URLQueryItem(
                name: "fields",
                value: "files(id,name,size,mimeType,modifiedTime,createdTime,md5Checksum,trashed),nextPageToken"
            ),
    ...
    public func move(from: CloudPath, to: CloudPath, account: CloudAccount) async throws -> CloudFileItem {
        try await rename(path: from, newName: to.lastComponent, account: account)
    }
    ...
    public func copy(from: CloudPath, to: CloudPath, account: CloudAccount) async throws -> CloudFileItem {
        ...
        var request = HTTPRequest(
            url: URL(string: "\(Self.baseURL)/files/\(fileID)/copy") ?? URL(fileURLWithPath: "/"),
            method: .POST
        )
        request.headers["Authorization"] = "Bearer \(token)"
        request.headers["Content-Type"] = "application/json"
    ...
    public func delete(path: CloudPath, account: CloudAccount) async throws {
        ...
        let fileID = path.lastComponent
        let request = HTTPRequest(
            url: URL(string: "\(Self.baseURL)/files/\(fileID)") ?? URL(fileURLWithPath: "/"),
            method: .DELETE,
            headers: ["Authorization": "Bearer \(token)"]
        )
        let response = try await http.data(for: request)
    ...
    public func fetchBlockManifest(path: CloudPath, account: CloudAccount) async throws -> BlockMap? {
        ...
        nil // Simplified — full impl would use Files.get?fields=appProperties
    }
    ...
    public func restoreFromTrash(item: CloudFileItem, account: CloudAccount) async throws {
        ...
        var request = HTTPRequest(
            url: URL(string: "\(Self.baseURL)/files/\(item.id)") ?? URL(fileURLWithPath: "/"),
            method: .PATCH
        )
        request.headers["Authorization"] = "Bearer \(token)"
        request.headers["Content-Type"] = "application/json"
    ...
    public func emptyTrash(account: CloudAccount) async throws {
        let token = try await accessToken(account: account)
        let request = HTTPRequest(
            url: URL(string: "\(Self.baseURL)/files/trash") ?? URL(fileURLWithPath: "/"),
            method: .DELETE,
            headers: ["Authorization": "Bearer \(token)"]
        )
        _ = try await http.data(for: request)
    ...
    public func listVersions(path: CloudPath, account: CloudAccount) async throws -> [FileVersion] {
        ...
        let request = HTTPRequest(
            url: URL(
                string: "\(Self.baseURL)/files/\(fileID)/revisions?fields=revisions(id,size,modifiedTime,keepForever)"
            ) ??
                URL(fileURLWithPath: "/"),
            headers: ["Authorization": "Bearer \(token)"]
        )
    ...
    public func createShareLink(
        ...
        var request = HTTPRequest(
            url: URL(string: "\(Self.baseURL)/files/\(fileID)/permissions") ?? URL(fileURLWithPath: "/"),
            method: .POST
        )
        request.headers["Authorization"] = "Bearer \(token)"
        request.headers["Content-Type"] = "application/json"
        ...
        _ = try await http.data(for: request)
        return ShareLink(
            url: URL(string: "https://drive.google.com/file/d/\(fileID)/view") ?? URL(fileURLWithPath: "/"),
            id: fileID
        )
    }
    ...
    private func accessToken(account: CloudAccount) async throws -> String {
        do {
            ...
            throw ProviderError
                .authenticationFailed("Could not obtain valid access token: \(error.localizedDescription)")
        }
    }
}
