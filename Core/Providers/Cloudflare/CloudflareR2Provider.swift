...
//
// R2 uses path-style URLs and the "auto" pseudo-region.
...
public actor CloudflareR2Provider: CloudProvider {
    ...
    public nonisolated let displayName = "Cloudflare R2"
    public nonisolated let iconName = "cloudflare"
    public nonisolated let capabilities: ProviderCapabilities

    // MARK: - Internals

    private let inner: S3Provider
    private let logger = Logger(subsystem: "com.stratus.cloudmanager", category: "CloudflareR2Provider")

    // MARK: - Init

    /// Creates a `CloudflareR2Provider` for the given bucket and Cloudflare account.
    ///
    /// - Parameters:
    ...
    public func quota(for account: CloudAccount) async throws -> StorageQuota {
        // R2 does not expose a quota API via S3-compatible endpoints.
    ...
    // MARK: - Trash (R2 has no trash concept)
