...
public actor OneDriveProvider: CloudProvider {
    ...
    public func validateCredentials(account: CloudAccount) async throws -> Bool {
        ...
        let request = HTTPRequest(
            url: URL(string: "\(Self.baseURL)") ?? URL(fileURLWithPath: "/"),
    ...
    public func quota(for account: CloudAccount) async throws -> StorageQuota {
        ...
        let request = HTTPRequest(
            url: URL(string: "\(Self.baseURL)?$select=quota") ?? URL(fileURLWithPath: "/"),
        ...
        return StorageQuota(
            totalBytes: quota["total"] as? Int64,
            usedBytes: (quota["used"] as? Int64) ?? 0,
    ...
    public func listDirectory(
        ...
        let url: URL = if let pt = pageToken, let nextURL = URL(string: pt) {
            nextURL
        } else {
            URL(
                string: "\(Self.baseURL)/\(itemPath)/children?$select=id,name,size,file,folder,lastModifiedDateTime,@microsoft.graph.downloadUrl&$top=999"
            ) ??
                URL(fileURLWithPath: "/")
        }
    ...
    public func fileMetadata(path: CloudPath, account: CloudAccount) async throws -> CloudFileItem {
        let token = try await accessToken(account: account)
        let request = HTTPRequest(
            url: URL(string: "\(Self.baseURL)/root:\(path.path)") ?? URL(fileURLWithPath: "/"),
            headers: ["Authorization": "Bearer \(token)"]
        )
        let response = try await http.data(for: request)
        guard response.isSuccess,
    ...
    public func completeMultipartUpload(
        ...
        CloudFileItem(id: "", name: "", path: CloudPath("/"))
    }
    ...
    public func copy(from: CloudPath, to: CloudPath, account: CloudAccount) async throws -> CloudFileItem {
        try await move(from: from, to: to, account: account)
    }
