...
final class BandwidthSaturationTests: XCTestCase {
    ...
    func test_high_bandwidth_saturation() async throws {
    ...
    func test_low_bandwidth_stable() async throws {
        let monitor = BandwidthMonitor()
        let controller = CongestionController()
    ...
    func test_convergence_to_stable_parallelism() async throws {
        let controller = CongestionController()
    ...
    func test_bandwidth_fairness_across_monitors() async throws {
        ...
        await withTaskGroup(of: Void.self) { group in
            group.addTask {
                for _ in 0..<20 { await m1.recordBytes(chunk, elapsed: elapsed) }
            }
            group.addTask {
                for _ in 0..<20 { await m2.recordBytes(chunk, elapsed: elapsed) }
            }
        }
