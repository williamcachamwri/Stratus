...
final class ConflictResolverTests: XCTestCase {
    ...
    private func makeConflict(localDate: Date, remoteDate: Date, localSize: Int64 = 100, remoteSize: Int64 = 200) -> SyncConflict {
        SyncConflict(
            pairID: UUID(),
    ...
    func test_keepLocal_alwaysUploads() async throws {
        ...
        let action = try await resolver.resolve(conflict: conflict, resolution: .keepLocal, provider: MockProvider(), account: makeAccount())
        if case .upload(_, _) = action { } else { XCTFail("Expected upload action") }
    }

    func test_keepRemote_alwaysDownloads() async throws {
        let conflict = makeConflict(localDate: Date(), remoteDate: Date.distantPast)
        let action = try await resolver.resolve(conflict: conflict, resolution: .keepRemote, provider: MockProvider(), account: makeAccount())
        if case .download(_, _) = action { } else { XCTFail("Expected download action") }
    }
    ...
    func test_keepNewer_localWins() async throws {
        ...
        if case .upload(_, _) = action { } else { XCTFail("Expected upload for newer local") }
    }

    func test_keepNewer_remoteWins() async throws {
        let conflict = makeConflict(localDate: Date.distantPast, remoteDate: Date())
        let action = try await resolver.resolve(conflict: conflict, resolution: .keepNewer, provider: MockProvider(), account: makeAccount())
        if case .download(_, _) = action { } else { XCTFail("Expected download for newer remote") }
    }
    ...
    func test_keepLarger_localLarger() async throws {
        ...
        if case .upload(_, _) = action { } else { XCTFail("Expected upload for larger local") }
    }
    ...
    func test_keepBoth_producesBothActions() async throws {
        ...
        if case .keepBoth(_, _, _, _) = action { } else { XCTFail("Expected keepBoth action") }
    }

    func test_askUser_returnsNeedsDecision() async throws {
        let conflict = makeConflict(localDate: Date(), remoteDate: Date())
        let action = try await resolver.resolve(conflict: conflict, resolution: .askUser, provider: MockProvider(), account: makeAccount())
...
private actor MockProvider: CloudProvider {
    ...
    nonisolated let capabilities = ProviderCapabilities(supportsMultipartUpload: false, supportsResumeUpload: false, supportsParallelChunks: false, maxChunkSize: 1024, minChunkSize: 1, maxConcurrentUploads: 1, multipartThresholdBytes: .max)
    ...
    func fileMetadata(path: CloudPath, account: CloudAccount) async throws -> CloudFileItem { CloudFileItem(id: "", name: "", path: path) }
    ...
    func uploadSmallFile(data: Data, remotePath: CloudPath, account: CloudAccount, metadata: UploadMetadata) async throws -> CloudFileItem { CloudFileItem(id: "", name: "", path: remotePath) }
    func downloadURL(path: CloudPath, account: CloudAccount, expiresIn: TimeInterval) async throws -> URL { URL(string: "https://example.com")! }
    ...
    func createShareLink(path: CloudPath, account: CloudAccount, options: ShareOptions) async throws -> ShareLink { ShareLink(url: URL(string: "https://example.com")!, id: "") }
    func revokeShareLink(link: ShareLink, account: CloudAccount) async throws {}
    func streamingURL(path: CloudPath, account: CloudAccount) async throws -> URL { URL(string: "https://example.com")! }
}
